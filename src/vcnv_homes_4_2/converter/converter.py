...
class Converter(AppObject):
    ...
    def output_csv_data(self, rooms, room_count):
        ...
        try:
            ...
            file_path = os.path.join(SystemInfo.get_instance().panorama_output_dir, 'panorama.csv')
            panorama_csv_file = open(file=file_path, mode='w', encoding='shift_jis',)
            ...
            if room_count > 0:
                ...
                for room in rooms:
                    ...
                    if room_data.has_panoramas:
                        panorama_csv_file.write(self.get_panorama_data_record(room_data))
    ...
    def get_panorama_header_record(cls):
        """ パノラマのヘッダーレコードの取得 """
        ans = '"{0}"'.format('header')  # ヘッダ識別文字列
        ans += ',"{0}"'.format(SystemInfo.get_instance().panorama_csv_version)  # バージョン番号
        ans += ',"{0}"'.format(SystemInfo.get_instance().homes_id)  # HOME'S会員番号
        ans += ',"{0}"'.format('0')  # 文字コード
        ans += ',"{0}"'.format(SystemInfo.get_instance().panorama_test_mode)  # テストモード
    ...
    def get_data_record(cls, room_data):
        ...
        if room_data:
            ...
            ans += ',"{0}"'.format('')  # リフォーム備考
            ans += ',"{0}"'.format(SystemInfo.get_instance().online_preview)  # オンライン内見可
            ans += ',"{0}"'.format(SystemInfo.get_instance().online_advice)  # オンライン相談可
            ans += ',"{0}"'.format(SystemInfo.get_instance().it_description)  # IT重説可
    ...
    @classmethod
    def get_panorama_data_record(cls, room_data):
        """パノラマデータレコードの取得"""
        ans = ''

        if room_data:
            ans += '"{0}"'.format(room_data.panorama_local_code)  # パノラマローカルID
            ans += ',"{0}"'.format(room_data.panorama_set_name)  # パノラマセット名
            ans += ',"{0}"'.format('1')  # 処理分類
            panorama = room_data.next_panorama
            ans += ',"{0}"'.format(room_data.get_panorama_filename(panorama))  # ファイル名１
            ans += ',"{0}"'.format(room_data.get_panorama_lens_type(panorama))  # レンズ種類１
            ans += ',"{0}"'.format(room_data.get_panorama_type_code(panorama))  # タイトル番号１
            panorama = room_data.next_panorama
            ans += ',"{0}"'.format(room_data.get_panorama_filename(panorama))  # ファイル名２
            ans += ',"{0}"'.format(room_data.get_panorama_lens_type(panorama))  # レンズ種類２
            ans += ',"{0}"'.format(room_data.get_panorama_type_code(panorama))  # タイトル番号２
            panorama = room_data.next_panorama
            ans += ',"{0}"'.format(room_data.get_panorama_filename(panorama))  # ファイル名３
            ans += ',"{0}"'.format(room_data.get_panorama_lens_type(panorama))  # レンズ種類３
            ans += ',"{0}"'.format(room_data.get_panorama_type_code(panorama))  # タイトル番号３
            panorama = room_data.next_panorama
            ans += ',"{0}"'.format(room_data.get_panorama_filename(panorama))  # ファイル名４
            ans += ',"{0}"'.format(room_data.get_panorama_lens_type(panorama))  # レンズ種類４
            ans += ',"{0}"'.format(room_data.get_panorama_type_code(panorama))  # タイトル番号４
            panorama = room_data.next_panorama
            ans += ',"{0}"'.format(room_data.get_panorama_filename(panorama))  # ファイル名５
            ans += ',"{0}"'.format(room_data.get_panorama_lens_type(panorama))  # レンズ種類５
            ans += ',"{0}"'.format(room_data.get_panorama_type_code(panorama))  # タイトル番号５
            ans += ',"{0}"'.format('1')  # レコード終了マーク
            ans += '\n'
